...
class Find extends BaseDBInterfaceHelper {
  ...
  _validate(context) { // eslint-disable-line no-unused-vars
    ...
    return new Promise((resolve, reject) => { // eslint-disable-line no-unused-vars
      ...
      if (!validate(job.payload.filter.limit, { type: 'number', optional: true, defaultTo: undefined }).isValid) {
        reject(new Error('missing/incorrect \'limit\' Number in job payload.filter'));
      }

      if (!validate(job.payload.filter.offset, { type: 'number', optional: true, defaultTo: undefined }).isValid) {
  ...
  _process(context) {
    ...
    output.on('done', function(brickname, response) {
      response = response.map(function(schedule) {
