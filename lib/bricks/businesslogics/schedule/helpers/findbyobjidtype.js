...
class FindByObjIdType extends BaseHelper {
   ...
   */
  _process(context) {
    this.findByObjIdType(context.data.payload.objId, context.data.payload.type)
      .then((scheduleObj) => {
      ...
      .catch((err) => {
        if(err instanceof Error) {
          ...
          this.logger.error(`Cannot find schedule by objId/type ${err.returnCode}: ${err.brickName} ${err.response}`)
