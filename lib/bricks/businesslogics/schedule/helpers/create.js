...
class Create extends BaseHelper {
  ...
  _process(context) {
    ...
    this.saveScheduleIfNotExists(schedule)
      .then((scheduleObj) => {
      ...
      .catch((err) => {
        if(err instanceof Error) {
          that.logger.error(`Cannot create schedule in DB ${err}`);
          context.emit('error', this.cementHelper.brickName, err);
        } else {
          that.logger.error(`Cannot create schedule in DB ${err.returnCode} ${err.brickName} ${err.response}`);
