...
class Delete extends BaseHelper {
  ...
  _process(context) {
    const that = this;
    this.deleteById(context.data.payload.id)
      .then((deletedSchedule) => {
        that.synchronizer.broadcast(context.data.nature.quality, context.data.payload, function(returnCode, brickName, response) {
          if(returnCode !== 'done') {
            ...
            context.emit(returnCode, that.cementHelper.brickName, deletedSchedule);
          }
        });
      })
      .catch((err) => {
        if(err instanceof Error) {
          ...
          that.logger.error(`Cannot delete schedule in DB ${err.returnCode} ${err.brickName} ${err.response}`);
